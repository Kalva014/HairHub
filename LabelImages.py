...
def label_image(folder_path, image_to_label):
    ...
    plt.show()
    #print(np.shape(img))
    #print(np.shape(hair_mask))
    hair_roi_pixels = img[hair_mask] # This gets pixels of region of interest
    extra_roi_pixels = img[extra_mask]

    create_labeled_file(hair_roi_pixels, image_to_label, os.path.join(os.getcwd(), 'data/segmented_imgs/medium_hairstyles'))
...
def main():
    ...
    for filename in os.listdir(folder):
        ...
        if image_file_name not in os.listdir('data/segmented_imgs/medium_hairstyles'): # Check if file is already labeled
